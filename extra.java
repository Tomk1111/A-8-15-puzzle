...
public class is17185203
{
	public static class dick
	{
		...
		public dick(int[][]state,dick parent)
		{
			this.state=state;
			this.parent=parent;
		}
		
		public dick(int [][]state)
		{
			this.state=state;
			this.parent=null;
		}
		
		public dick getParent()
		{
			return parent;
		}
	...
	private static ArrayList<dick> path=new ArrayList<dick>();//path to final state
...
public static void main(String[] args)
{
	...
	openLevel.add(0);
	//printBoard(finall);
	//printBoard(path.get(0).getState());
    solveit(finall,posx,posy,goal,path.get(0));

	/*for(int gg=0;gg<path.size();gg++)
	{
		printBoard(path.get(gg).getState());
		System.out.println();
	}*/
	dick ddd=path.get(path.size()-1);
	while(ddd.getParent()!=null)
	{
		printBoard(ddd.getState());
		ddd=ddd.getParent();
		System.out.println();
	}
}
...
public static void solveit(int[][]source,int x,int y,int [][]goal,dick d)
{
	try
	{	
		...
		//	System.out.println();
			...
			if(y+1<source.length)
				...
				solveit(newSource, posX, posY, goal,d);
